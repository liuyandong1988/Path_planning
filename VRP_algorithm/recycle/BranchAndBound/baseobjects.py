...
class Fleet(object):
    ...
    def search_id_for_node(self, node_id):
        for vehicle in self.fleet:
            ...
            return None
    # find the node in which vehicle
...
class Route(object):
    ...
    # the node position in the route
...
def printRuselt(instance):
    result = {}
    for i, vehicle in enumerate(instance.solution.fleet):
        vehicle_path = []
        for node in vehicle.route:
            vehicle_path.append(node.id)
        result[i] = vehicle_path 
    print (result)
