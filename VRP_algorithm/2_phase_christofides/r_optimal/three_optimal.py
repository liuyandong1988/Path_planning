...
def three_optimal(route, distance_matrix):
    ...
    while not local_optimal:
                ...
                for k in range(0, route_length):
                    if (k > j > i and k != j + 1 and j != i + 1 and i != k + 1) or \
                        ...
                        for case in eight_case:
                            # gain_cost, current_case = gain_from_three_optimal(X1-1, X2-1, Y1-1, Y2-1, Z1-1, Z2-1, case, distance_matrix)  # if distance matrix excludes 0 node minus 1
                            gain_cost, current_case = gain_from_three_optimal(X1, X2, Y1, Y2, Z1, Z2, case, distance_matrix)
        ...
        if not local_optimal:
            ...
            tmp = calculate_cost(route, distance_matrix)
            # print('*', best_cost, best_case)
            # print('Distance %s Route: %s' % (tmp, route))
