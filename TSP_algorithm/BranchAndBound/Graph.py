...
#             coordination.append([i+1, int(x), int(y)])
#         
# #         print coordination
# #         raw_input('prompt')
#         # write the distance
#         for i in xrange(theGraph.size()):
#             (num, i_x, i_y) = tspFile.readline().split()
#             locations.append((float(i_x), float(i_y)))
#         for i in xrange(theGraph.size()): 
#             for j in xrange(theGraph.size()): 
#                 if i == j:
#                     pass
#                 else:
#                     distance = math.sqrt((locations[i][0] - locations[j][0])**2 + (locations[i][1] - locations[j][1])**2)
#                     theGraph.setAt(i,j, int(distance))
...
#     newGraph = Graph(['Portland','Newberg','Dundee','Beaverton','Richland','Seattle'], None, None, True, False)
    newGraph = Graph(['1','2','3','4','5','6'], None, None, True, False)
